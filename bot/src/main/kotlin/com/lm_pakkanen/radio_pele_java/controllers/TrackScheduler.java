...
public final class TrackScheduler {
  ...
  public boolean isPlaying() {
    return LavaLinkUtil.Companion.getPlayer(guildId).getTrack() != null;
  }
  ...
  private Optional<Track> playNextTrack() {
    ...
    if (this.store.getQueueSize() > 0 || !this.store.hasPlaylist()) {
      ...
      nextTrackOpt.ifPresentOrElse(nextTrack -> {
        ...
        LavaLinkUtil.Companion.getPlayer(guildId).setTrack(nextTrack).subscribe();
  ...
  private void stopCurrentSong() {
    ...
    LavaLinkUtil.Companion.getPlayer(guildId).setPaused(false).setTrack(null).subscribe();
  }
