...
class SpotifyController(
    ...
    fun resolveQualifiedTrackNames(url: String?): MutableList<String> {
        ...
        try {
            ...
            if (isAlbum) {
                val albumSimplifiedTracks =
                    this.spotifyApi
                        .getAlbumsTracks(entityId)
    ...
    private fun refreshAccessToken() {
        try {
             ...
             * token expires
             */
            val nextRefreshDelayInSeconds = (
                nextRefreshExpiresInSeconds -
                    5 * 60
            )

            val nextRefreshDelayInMilliseconds = (
                nextRefreshDelayInSeconds
                    * 1000
            )

            val refreshAccessTokenTimer = Timer()

            val refreshAccessTokenTimerTask: TimerTask =
                object : TimerTask() {
                    override fun run() {
                        refreshAccessToken()
                    }
