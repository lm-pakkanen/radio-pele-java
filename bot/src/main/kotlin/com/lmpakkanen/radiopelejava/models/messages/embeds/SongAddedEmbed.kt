...
import com.lmpakkanen.radiopelejava.models.RefinedTrackInfo
import com.lmpakkanen.radiopelejava.models.Store
import dev.arbjerg.lavalink.client.player.Track
import net.dv8tion.jda.api.entities.MessageEmbed

class SongAddedEmbed(
    track: Track,
    store: Store,
) : IEmbedBuilder {
    ...
    init {
        ...
        val trackTitleWithDuration =
            trackTitleWithDurationBuilder
                .toString()

        val queueSize = store.getQueueSize()
        val playlistQueueSize = store.getPlaylistQueueSize()

        val queueLengthDescription =
            if (store.hasPlaylist() && queueSize == 0) {
